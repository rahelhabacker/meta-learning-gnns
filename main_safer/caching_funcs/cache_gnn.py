...
class CacheGNNEmbeds:
    def __init__(self, config, model):
        self.config = config
        self.model = model
        # self.model_file = config['model_file']
        self.data_name = config['data_name']
        self.comp_dir = os.path.join('data', 'complete_data', self.data_name)
        self.data_dir = os.path.join('FakeHealth')

    def predict_and_cache(self, data_name):
        if data_name in ['gossipcop', 'politifcat']:
            self.predict_and_cache_fakenews()
        elif data_name in ['HealthStory', 'HealthRelease']:
    ...
    def obtain_user_representations(self):
        ...
        self.user_cache = torch.zeros(len(self.node2id), self.config['embed_dim']) if self.config['model_name'] not in [
            'gat', 'rgat'] else torch.zeros(len(self.node2id), 3 * self.config['embed_dim'])

        with torch.no_grad():
            self.model.eval()
            if self.config['full_graph']:
                _, _, node_embeds = self.model(self.config['data'].x.to(device),
        ...
        name = 'user_embeds_graph_lr' if self.config['data_name'] in ['HealthRelease',
                                                                      'HealthStory'] else 'user_embeds_graph_lr_30_30'
        user_embed_file = os.path.join(self.comp_dir, 'cached_embeds',
                                       '{}_{}_{}.pt'.format(name, self.config['seed'], self.config['model_name']))
    ...
    def obtain_doc_representations(self):
        ...
        for split in splits:
            ...
            split_doc_cache = torch.zeros(len(self.node2id), self.config['embed_dim']) if self.config[
                                                                                              'model_name'] != 'gat' else torch.zeros(
                len(self.node2id), 3 * self.config['embed_dim'])
            ...
            for count, doc in enumerate(split_docs):
                if self.config['data_name'] in ['gossipcop', 'politifact']:
                    ...
                    doc_file = os.path.join(self.data_dir, 'engagements', 'complete', self.config['data_name'],
                                            str(doc) + '.json')
            ...
            name = 'doc_embeds_graph_poinc_wt3_lr' if self.config['data_name'] in ['HealthRelease',
                                                                                   'HealthStory'] else 'doc_embeds_graph_lr_30_30_poinc'
            doc_embed_file = os.path.join(self.comp_dir, 'cached_embeds',
                                          '{}_{}_{}_{}.pt'.format(name, split, self.config['seed'],
